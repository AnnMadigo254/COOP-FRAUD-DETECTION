...
"""
Fraud Detection API - KAFKA-ONLY MODEL (37 features)
Updated to work with model trained on Kafka-only data

Features from Kafka:
- Basic (5): amount, hour, day, weekend, night
- Transaction types (20): All payment types
- Channels (6): All 6 channels found
- Additional (6): status_failed, foreign_currency, etc.
"""
...
class KafkaOnlyFraudDetectionService:
    """Fraud detection using Kafka-only trained model"""
    
    # All 6 channels from Kafka analysis
    CHANNEL_IDS = [
        '2E08D0409BE26BA3E064020820E70A68',
        '2E08D0409BE76BA3E064020820E70A68',
        '2E08D0409BE16BA3E064020820E70A68',
    ...
    # All 20 payment types from Kafka analysis
    PAYMENT_TYPES = [
        'PAY_TO_COOPTILL',
        'POST_DIRECT_PAYMENTS',
        'PERSONAL_ELOAN',
    ...
    # Enhanced rule thresholds
    VELOCITY_THRESHOLD = 5
    FAILED_ATTEMPTS_THRESHOLD = 3
    HIGH_AMOUNT_THRESHOLD = 50000
    CRITICAL_AMOUNT_THRESHOLD = 100000
    ...
    def __init__(self, model_path: str, scaler_path: str, config_path: str):
        ...
        logger.info(f" Kafka-Only Fraud Detection loaded")
        logger.info(f"   Model version: {self.config.get('model_version', 'Unknown')}")
        logger.info(f"   Features: {len(self.feature_columns)}")
        logger.info(f"   Data source: {self.config.get('data_source', 'Unknown')}")
        logger.info(f"   Max distance: {self.max_distance:.2f}")
    ...
    def _calculate_max_distance(self):
        ...
        return max_dist * 1.5  # Add 50% buffer
    ...
    def extract_time_features(self, transaction_date: str) -> Dict[str, int]:
        """Extract time features from transaction date"""
        try:
            if transaction_date:
                dt = datetime.fromisoformat(transaction_date.replace('Z', '+00:00'))
            else:
            ...
            logger.warning(f"Error extracting time: {str(e)}")
            return {'hour_of_day': 12, 'day_of_week': 3, 'is_weekend': 0, 'is_night': 0}
    ...
    def apply_enhanced_rules(self, transaction_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # RULE 1: SIM SWAP (if phone_mismatch provided)
        phone_mismatch = transaction_data.get('phone_mismatch', 0)
        if phone_mismatch == 1:
            triggered_rules.append("SIM_SWAP_DETECTED")
            auto_block = True
        ...
        return {
            'auto_block': auto_block,
            'risk_boost': min(risk_boost, 3),
            'triggered_rules': triggered_rules,
            'enhanced_score': min(enhanced_score, 1.0)
        }
    
    def prepare_features(self, transaction_data: Dict[str, Any]) -> np.ndarray:
        """Prepare 37 features matching Kafka-only training"""
        ...
        txn_type = transaction_data.get('transaction_type', 'PAY_TO_COOPTILL')
        channel_id = transaction_data.get('channelId', self.CHANNEL_IDS[0])
        channel = transaction_data.get('channel', '12')
        
        # Status and currency
        status_val = transaction_data.get('status', 1)
        currency = transaction_data.get('transaction_currency', 'KES')
        
        # Device and phone (for hashing)
        device_id = transaction_data.get('device_id', 'UNKNOWN')
        phone_number = transaction_data.get('phone_number', '')
        
        # Credit account
        credit_account = transaction_data.get('credit_account_id', '')
        ...
        features['status_failed'] = 1 if status_int == 0 else 0
        features['is_foreign_currency'] = 1 if currency != 'KES' else 0
        features['has_credit_account'] = 1 if credit_account else 0
        features['channel_numeric'] = int(channel) if str(channel).isdigit() else 12
        features['device_hash'] = hash(device_id) % 1000
        features['unique_phone'] = hash(phone_number) % 100
        
        # Convert to array using feature_columns order
        feature_array = np.array([
            features.get(col, 0) for col in self.feature_columns
        ]).reshape(1, -1).astype(float)
    ...
    def predict(self, transaction_data: Dict[str, Any]) -> Dict[str, Any]:
        """Predict fraud using ML + rules"""
        
        # Step 1: Apply rules
        rule_results = self.apply_enhanced_rules(transaction_data)
        
        # Step 2: Auto-block check
        if rule_results['auto_block']:
            return {
                'fraud_prediction': 1,
                'fraud_score': 1.0,
        ...
        try:
            ...
            cluster_id = self.model.predict(X_scaled)[0]
            cluster_center = self.model.cluster_centers_[cluster_id]
            raw_distance = np.linalg.norm(X_scaled[0] - cluster_center)
            
            # Normalize distance
            normalized_distance = self.normalize_distance(raw_distance)
            
            # Combine scores
            ml_score = normalized_distance
            enhanced_score = rule_results['enhanced_score']
            combined_score = min(ml_score + enhanced_score, 1.0)
            
            # Fraud threshold
            threshold = 0.5
            is_fraud = int(combined_score > threshold)
            ...
            boosted_index = min(base_index + rule_results['risk_boost'], 3)
            final_risk = risk_levels[boosted_index]
...
fraud_detector = KafkaOnlyFraudDetectionService(
    model_path=str(MODEL_DIR / 'fraud_detector_production.pkl'),
    scaler_path=str(MODEL_DIR / 'scaler_production.pkl'),
    config_path=str(MODEL_DIR / 'feature_config_production.pkl')
)
    ...
    def post(self, request):
        ...
        try:
            ...
            response_data = {
                ...
                'cluster_id': result.get('cluster_id', -1),
                'distance': result.get('distance', 0),
                'raw_distance': result.get('raw_distance', 0),
                'triggered_rules': result.get('triggered_rules', []),
                'detection_method': result.get('detection_method', 'ML_ONLY'),
                'timestamp': datetime.now().isoformat(),
                'processing_time_ms': round(processing_time, 2)
            ...
            logger.info(
                f" TXN={response_data['transaction_id']}, "
                f"Amount={request.data.get('transaction_amount')}, "
                f"Raw={result.get('raw_distance', 0):.2f}, "
                f"Norm={result.get('distance', 0):.4f}, "
    ...
    def get(self, request):
        try:
            ...
            return Response({
                ...
                'features': len(fraud_detector.feature_columns),
                'max_distance': round(fraud_detector.max_distance, 2),
                'performance': {
                ...
                'enhanced_rules': {
                    'velocity_monitoring': f'{KafkaOnlyFraudDetectionService.VELOCITY_THRESHOLD} txn/day',
                    'failed_attempts': f'{KafkaOnlyFraudDetectionService.FAILED_ATTEMPTS_THRESHOLD} attempts/hour',
                    'device_tracking': 'ENABLED',
                    'night_transactions': 'ENABLED'
                }
            }, status=status.HTTP_200_OK)
